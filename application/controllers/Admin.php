...
class Admin extends CI_Controller {
    ...
    public function users_payment($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        $page_data['page_name'] = 'users_payments';
        $page_data['page_title'] = get_phrase('users_payments');
        $page_data['courses'] = $this->crud_model->get_all_courses_by_active();

        if ($param1 == "add") {
            $this->user_model->add_user();
            redirect(site_url('admin/users_payments'), 'refresh');
        }

        if ($param1 == "search") {
            $this->user_model->add_user();
            $page_data['users'] = $this->lazyload->get_all_outstanding_payment_by_course($this->input->post("course_id"))->result_array();
            $this->load->view('backend/index', $page_data);
            return;
        }
    ...
    public function user_payment_form($param1 = "", $param2 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'add_user_payment_form') {
            $page_data['page_name'] = 'user_payment_add';
            $page_data['page_title'] = get_phrase('student_add');
            $page_data['users'] = $this->user_model->get_all_students()->result_array();
            $page_data['courses'] = $this->crud_model->get_all_courses_by_active();
            $this->load->view('backend/index', $page_data);
        }
        elseif ($param1 == 'edit_user_payment_form') {
            $page_data['page_name'] = 'user_payment_edit';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = get_phrase('student_edit');
            $page_data['courses'] = $this->crud_model->get_all_courses_by_active();
            $this->load->view('backend/index', $page_data);
